...
//  RECONCILIATION FLOW
//
//  After each successful statement import, this flow opens (only if
//  the matcher found candidates) and asks the user to confirm each
//  "manual quick-entry ←→ imported charge" pair. The user picks one
//  of three actions per pair:
//
...
function _walkQueue() {
  while (_queue.length > 0) {
    ...
    _activePairs  = pairs;
    _decisions    = new Map(pairs.map(p => [_key(p), 'merge']));   // default = accept
    _open = true;
    _renderModal();
    return;
  }
...
function _commitDecisions() {
  ...
  for (const pair of _activePairs) {
    ...
    if (decision === 'merge') {
      // Transfer user enrichment from manual → imported, then drop
      // the manual. We prefer the manual's values when set so the
      // user's quick-entry intent wins over any blank field on the
      // imported side.
      const importedIdx = charges.findIndex(c => c.id === pair.imported.id);
      if (importedIdx === -1) continue;
